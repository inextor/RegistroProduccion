...
export class RestService {
  ...
  async getProductionAreaItems(productionAreaId: number): Promise<any> {
    const url = `${this.baseUrl}/production_area_item.php?production_area_id=${productionAreaId}&status=ACTIVE&limit=99999`;
    try {
      const response = await fetch(url, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        const errorData = await response.text();
        throw new Error(`HTTP error fetching production area items: ${response.status}, message: ${errorData}`);
      }
      return await response.json();
    } catch (error) {
      console.error(`Error in getProductionAreaItems for production area ID ${productionAreaId}:`, error);
      throw error;
    }
  }
